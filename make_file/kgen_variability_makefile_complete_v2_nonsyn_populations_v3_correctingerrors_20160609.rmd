...
rm(flipped_bind, kgen, kgen.imp, starp, pos, count_EUR, count_SAS, count_EAS, count_AMR, count_AFR, count_all)
...
colnames(shorted.df)[(names(shorted.df) == "n")] <- "total_exons"

#Save file
write.csv(shorted.df, file = "variability_annotated_nonsyn_R_population_20160608.csv", quote=FALSE)

```

#Load saved file
```{r reload}
shorted.df <- read.table("variability_annotated_nonsyn_R_population_20160608.csv", header=TRUE, sep= ",")
shorted.df$X <- NULL
colnames(shorted.df)[(names(shorted.df) == "total_exons")] <- "count_AFR"
colnames(shorted.df)[(names(shorted.df) == "n")] <- "total_exons"
...
names <- colnames(shorted.df)
merge.scores.df <- merge(merge.df, dfs)
colnames(merge.scores.df)[2] <- "CHROM"
clean.df <- mutate(merge.scores.df, CHROM=str_replace_all(CHROM, pattern="chr", ""))
scores <- select(clean.df, contains("score"))
short.df <- clean.df[1:24]
bind <- bind_cols(short.df, scores)

#write csv
write.csv(bind, file= "variability_thresholds_nonsyn_populations_20160609.csv", quote=FALSE)
```